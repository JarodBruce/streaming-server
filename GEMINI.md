...
## プロジェクトの目的
RustlangとDockerを使ってYoutubeと同じようなサーバを作ってもらいます。
`./av`というフォルダに`.mp4`ファイルを入れておくのでそれをストリーミングし
クラアントはWebサイトからそれを再生することができる

そして同時接続人数をリアルタイムで表示する項目をつけてください
リアルタイムでこの動画を何人が同時にストリーミングしているかを表示してほしい

`main.rs`にはある程度のプログラムが書かれているのでそれを参考にどうぞ
## テスト
